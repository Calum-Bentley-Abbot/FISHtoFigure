...
def run(input_folder, hist, scatter, thresh, expression, dubposA, dubposAchans, dubposAthresh, dubposB, dubposBchans, dubposBthresh, chan_headers, dataset_labels):
        ...
        with redirect_stdout(f):                                                                    #redirects the console output to newly created file
            ...
            for a in range(len(path_list)):                                                                             #cycles through csv files created above, this constitutes the main loop for the script
                ...
                if Plot_Spot_Map == 1:
                    ...
                    size = []
                    plt.xlim(0, max(x_centroids)+10)
                    plt.ylim(0, max(y_centroids)+10)                                                                                                                                        #List which will be used to store spot information to be plotted following thresholding 
    ...
    if Plot_RNA_expression == 1:
        ...
        dfspot.to_csv(f"{output_folder}/General Transcript Abundance Analytics/Cell-wise Transcript Count by Channel.csv", sep=',')
    ...
    if Plot_dubposA == 1:
        ...
        try:
            ...
            plt.xticks(plt.xticks()[0])
            plt.legend(custom_lines, datasetlabels)
            plt.savefig(f"{output_folder}/[{dubposAchans}] Positive Cells/Total Transript Count [{dubposAchans}]", bbox_inches = "tight", dpi=500)
