...
def run(input_folder, hist, scatter, thresh, expression, dubposA, dubposAchans, dubposAthresh, dubposB, dubposBchans, dubposBthresh, chan_headers, dataset_labels):
            ...
            for a in range(len(path_list)):                                                                             #cycles through csv files created above, this constitutes the main loop for the script
                ...
                with open(input_file) as file:                                                                          #Opens the current file
                    ...
                    for header in headers:                                                                                      #Cycles through all headers in the creted headers list
                        if re.search('Centroid X.+', header):                                                                   #searches the headers list for names of columsn of interest
                ...
                for i in range(len(intensity_cols_sorted)):                                     #This creates the list of lists, each internal list of numerical elements contains information about a particular cell. Each numerical element is the intensity for each subcellular spot in that cell. Lists containing channel headers seperate each set of cellular spot information lists by channel
                    ...
                    for row in data_in:                                                         #Here, we iterate over the rows in our dataset, the dataset is structured such that a row contains information relating to a cell, then subsequent rows contain information relating to any subcellular spots/clusters in that cell until all subcellular objects have been reported when the next row then pertains to the next cell, thus we refer here to data_in since the output data proccessed above contains only cellular rows and eliminated subcellular information.
                        if row[1] == 'PathCellObject':                                          #If the row represents a cell rather than a subcellular object
                        ...
                        elif row[1].startswith("Subcellular spot:"):                            #Since subcellular rows can contain either spot information or cluster information this provides a filter to remove any rows containing cluster information.
                ...
                #Reshaping cellular intensity dataframe form plotting purposes
                ...
                for row in data_in:                                                                                     #This loop writes the headers to the new file
                    if lc == 0:
                        ...
                        if row[1] == 'PathCellObject':                                                                  #Eliminates any subcellular rows, these are processed below seperately
                            for i in req_cols:
                                if row[i] != '0' and row[i] != 'NaN':                                                   #Eliminates any rows in which all columns of interest contain no data
                ...
                print(f"Percentage of cells with non-zero spot count = {nz_cell_percentage}%")
                ...
                print(f"Mean spot count in non-zero cells = {nz_mean_spot_count}")                                                  
                print(f"Mean cluster count in non-zero cells = {nz_mean_cluster_count}")
                print(f"Mean spot count in all cells = {total_mean_spot_count}")
                ...
                dfout_thresh = dfout[dfout[col_labels_spots].sum(axis = 1) >= threshold]
                for i in range(len(col_labels)):                                                #Loop to rename columns based on user inputted channel names
                    dfout_thresh.rename(columns = {col_labels_spots[i]:col_labels[i]}, inplace = True)
                ...
                if Plot_Spot_Histogram == 1:
                    ...
                    dfout_hist = dfout[dfout[col_labels_spots] > 0]
                    for i in range(len(col_labels_spots)):
                    ...
                    plt.xlabel("Spot Count")                                                                                                                            #labels the axes and titles figure
                    plt.ylabel("Occurance")
                    plt.title(f"{datasetlabels[a]} Spot Count Occurances")
                    plt.savefig(f"{output_folder}{datasetlabels[a]} Spot Count Occurance Histogram")
                ...
                if Plot_Spot_Map == 1:
                    ...
                    for i in range(len(spot_headers)):                                                                                                                          #For each channel, plots the centroids of all cells and sizes them according to the number of spots in that cell (weights created above). Colours are then assigned automatically for each channels data.
                        plt.scatter(x = dfout_thresh["X Centroids"], y = dfout_thresh["Y Centroids"], s = 10*(dfout_thresh[col_labels].iloc[:,i]), alpha = 0.5, label = col_labels[i])                             #Creates the scatter plot of centroid data for the current channel, 's' is the size this allows us to make the size a function of the number of spots in this cell (the number of spots within this cell is being read from the spots dataframe we created above), 'label' simply names your data based on the channel being plotted
                        plt.xlabel("X Centroids", fontsize = 10)
                        plt.ylabel("Y Centroids", fontsize = 10)
                    plt.title(f"{datasetlabels[a]} Transcriptional Spot Distribution")
                    plt.xlim([0, dfout["X Centroids"].max()])
                    plt.ylim([0, dfout["Y Centroids"].max()])
                    ax.invert_yaxis()
                    ax.set_aspect('equal')                                                                                                               
                    ...
                    plt.savefig(f"{output_folder}{datasetlabels[a]} Spot Distribution by Channel")
    ...
    if Plot_RNA_expression == 1:
        ...
        plt.figure()
        sns.boxplot(data = dfspotlong, x = "Channel", y = "Spots", hue = "Dataset")
        sns.stripplot(data = dfspotlong, x = "Channel", y = "Spots", hue = "Dataset", dodge = True)
        plt.xticks(plt.xticks()[0])
        plt.xlabel("Channel")
        plt.ylabel("Single Spots")
        plt.title("Cell-wise Spot Count by Channel")
        plt.legend()
        plt.savefig(f"{output_folder}\\General Transcript Abundance Analytics\\Cell-wise Spot Count by Channel")
        dfspot.to_csv(f"{output_folder}\\General Transcript Abundance Analytics\\Cell-wise Spot Count by Channel.csv", sep=',')
        
        #Plot total cellular intensity per cell per channel
        
        plt.figure()
        sns.boxplot(data = dfintensitylong, x = "Channel", y = "Cell Intensity", hue = "Dataset")
        sns.stripplot(data = dfintensitylong, x = "Channel", y = "Cell Intensity", hue = "Dataset", dodge = True)
        plt.xticks(plt.xticks()[0])
        plt.xlabel("Channel")
        plt.ylabel("Intensity")
        plt.title("Cell-wise Intensity by Channel")
        plt.legend()
        plt.savefig(f"{output_folder}\\General Transcript Abundance Analytics\\Cell-wise Intensity by Channel")
        ...
        plt.figure()
        sns.boxplot(data = dfnzintensitylong, x = "Channel", y = "Cell Intensity", hue = "Dataset")
        sns.stripplot(data = dfnzintensitylong, x = "Channel", y = "Cell Intensity", hue = "Dataset", dodge = True)
        plt.xticks(plt.xticks()[0])
        plt.xlabel("Channel")
        plt.ylabel("Intensity")
        plt.title("Cell-wise Intensity for Non-Zero Cells by Channel")
        plt.legend()
        plt.savefig(f"{output_folder}\\General Transcript Abundance Analytics\\Intensity of Non-Zero Cells by Channel")
        ...
        plt.figure()
        sns.barplot(data = dfspottotallong, x = "Channel", y = "Spot Count", hue = "Dataset")
        plt.title("Total Spot Count by Channel")
        plt.xticks(plt.xticks()[0])
        plt.legend()
        plt.savefig(f"{output_folder}\\General Transcript Abundance Analytics\\Total Spot Count by Channel")
        dfspottotal.to_csv(f"{output_folder}\\General Transcript Abundance Analytics\\Total Spot Count by Channel.csv", sep=',')
        ...
        plt.figure()
        sns.barplot(data = dfintensitytotallong, x = "Channel", y = "Intensity", hue = "Dataset")
        plt.title("Total Intensity by Channel")
        plt.xticks(plt.xticks()[0])
        plt.legend()
        plt.savefig(f"{output_folder}\\General Transcript Abundance Analytics\\Total Intensity by Channel")
        ...
        plt.figure()
        sns.barplot(data = dfposcellslong, x = "Channel", y = "CellNum", hue = "Dataset")
        plt.title("Number of Spot-Positive Cells per Channel")
        plt.ylabel("Number of Cells")
        plt.xticks(plt.xticks()[0])
        plt.legend()
        plt.savefig(f"{output_folder}\\General Transcript Abundance Analytics\\Positive Cell Count by Channel")
        dfposcells.to_csv(f"{output_folder}\\General Transcript Abundance Analytics\\Positive Cell Count by Channel.csv", sep=',')
        ...
        plt.figure()
        sns.barplot(data = dfposcells_tot, x = "Dataset", y = "Value")
        plt.ylabel("Number of Cells")
        plt.title("Total Spot-Positive Cells (All Channels)")
        plt.savefig(f"{output_folder}\\General Transcript Abundance Analytics\\Positive Cell Count (All Channels)")
        dfposcells_tot.to_csv(f"{output_folder}\\General Transcript Abundance Analytics\\Positive Cell Count (All Channels).csv", sep=',')
    ...
    if Plot_dubposA == 1:
        ...
        plt.figure()
        sns.boxplot(data = dfdubposAspotlong, x = "Channel", y = "Spots", hue = "Dataset")
        sns.stripplot(data = dfdubposAspotlong, x = "Channel", y = "Spots", hue = "Dataset", dodge = True)
        plt.xticks(plt.xticks()[0])
        plt.xlabel("Channel")
        plt.ylabel("Single Spots")
        plt.title(f"Cell-Wise Spot Count for [{dubposAchans}] Positive Cells")
        plt.legend()
        plt.savefig(f"{output_folder}\\[{dubposAchans}] Positive Cells\\Cell-wise Spot Count [{dubposAchans}]")
        dfdubposAspot.to_csv(f"{output_folder}\\[{dubposAchans}] Positive Cells\\Cell-wise Spot Count [{dubposAchans}].csv", sep=',')

        #Box plot of cell-wise intensity for first double positive channel pair

        plt.figure()
        sns.boxplot(data = dfdubposAintensitylong, x = "Channel", y = "Cell Intensity", hue = "Dataset")
        sns.stripplot(data = dfdubposAintensitylong, x = "Channel", y = "Cell Intensity", hue = "Dataset", dodge = True)
        plt.xticks(plt.xticks()[0])
        plt.xlabel("Channel")
        plt.ylabel("Intensity")
        plt.title(f"Cell-wise Intensity for [{dubposAchans}] Positive Cells")
        plt.legend()
        plt.savefig(f"{output_folder}\\[{dubposAchans}] Positive Cells\\Cell-wise Intensity [{dubposAchans}]")
        ...
        plt.figure()
        sns.barplot(data = dfdubposAspottotallong, x = "Channel", y = "Spot Count", hue = "Dataset")
        plt.title(f"Total Spot Count for [{dubposAchans}] Positive Cells")
        plt.xticks(plt.xticks()[0])
        plt.legend()
        plt.savefig(f"{output_folder}\\[{dubposAchans}] Positive Cells\\Total Spot Count [{dubposAchans}]")
        dfdubposAspottotal.to_csv(f"{output_folder}\\[{dubposAchans}] Positive Cells\\Total Spot Count [{dubposAchans}].csv", sep=',')
    
        #Equivilant bar plot for total intensity per channel
    
        plt.figure()
        sns.barplot(data = dfdubposAintensitytotallong, x = "Channel", y = "Intensity", hue = "Dataset")
        plt.title(f"Total Intensity for [{dubposAchans}] Positive Cells")
        plt.xticks(plt.xticks()[0])
        plt.legend()
        plt.savefig(f"{output_folder}\\[{dubposAchans}] Positive Cells\\Total Intensity [{dubposAchans}]")
        ...
        plt.figure()
        sns.barplot(data = dfdubposAnumlong, x = "Channel", y = "Number of Cells", hue = "Dataset")
        plt.title(f"Total Number of [{dubposAchans}] Positive Cells by Target Channel")
        plt.xticks(plt.xticks()[0])
        plt.legend()
        plt.savefig(f"{output_folder}\\[{dubposAchans}] Positive Cells\\Positive Cell Number [{dubposAchans}]")
        ...
        plt.figure()
        sns.barplot(data = dfdubposAnum_tot, x = "Dataset", y = "Value")
        plt.ylabel("Number of Cells")
        plt.title(f"Total Number of [{dubposAchans}] Positive Cells (All Channels)")
        plt.savefig(f"{output_folder}\\[{dubposAchans}] Positive Cells\\Total Positive Cells [{dubposAchans}]")
    ...
    if Plot_dubposB == 1:
        ...
        plt.figure()
        sns.boxplot(data = dfdubposBspotlong, x = "Channel", y = "Spots", hue = "Dataset")
        sns.stripplot(data = dfdubposBspotlong, x = "Channel", y = "Spots", hue = "Dataset", dodge = True)
        plt.xticks(plt.xticks()[0])
        plt.xlabel("Channel")
        plt.ylabel("Single Spots")
        plt.title(f"Cell-Wise Spot Count for [{dubposBchans}] Positive Cells")
        plt.legend()
        plt.savefig(f"{output_folder}\\[{dubposBchans}] Positive Cells\\Cell-wise Spot Count [{dubposBchans}]")
        dfdubposBspot.to_csv(f"{output_folder}\\[{dubposBchans}] Positive Cells\\Cell-wise Spot Count [{dubposBchans}].csv", sep=',')
    
        #Box plot of cell-wise intensity for second double positive channel pair

        plt.figure()
        sns.boxplot(data = dfdubposBintensitylong, x = "Channel", y = "Cell Intensity", hue = "Dataset")
        sns.stripplot(data = dfdubposBintensitylong, x = "Channel", y = "Cell Intensity", hue = "Dataset", dodge = True)
        plt.xticks(plt.xticks()[0])
        plt.xlabel("Channel")
        plt.ylabel("Intensity")
        plt.title(f"Cell-wise Intensity for [{dubposBchans}] Positive Cells")
        plt.legend()
        plt.savefig(f"{output_folder}\\[{dubposBchans}] Positive Cells\\Cell-wise Intensity [{dubposBchans}]")
        dfdubposBintensity.to_csv(f"{output_folder}\\[{dubposBchans}] Positive Cells\\Cell-wise Intensity [{dubposBchans}].csv", sep=',')
    
        #Bar plot of double positive Spot Count (total spots in all cells) per channel per dataset
    
        plt.figure()
        sns.barplot(data = dfdubposBspottotallong, x = "Channel", y = "Spot Count", hue = "Dataset")
        plt.title(f"Total Spot Count for [{dubposBchans}] Positive Cells")
        plt.xticks(plt.xticks()[0])
        plt.legend()
        plt.savefig(f"{output_folder}\\[{dubposBchans}] Positive Cells\\Total Spot Count [{dubposBchans}]")
        dfdubposBspottotal.to_csv(f"{output_folder}\\[{dubposBchans}] Positive Cells\\Total Spot Count [{dubposBchans}].csv", sep=',')
    
        #Equivilant plot for total intensity per channel
    
        plt.figure()
        sns.barplot(data = dfdubposBintensitytotallong, x = "Channel", y = "Intensity", hue = "Dataset")
        plt.title(f"Total Intensity for [{dubposBchans}] Positive Cells")
        plt.xticks(plt.xticks()[0])
        plt.legend()
        plt.savefig(f"{output_folder}\\[{dubposBchans}] Positive Cells\\Total Intensity [{dubposBchans}]")
        dfdubposBintensitytotal.to_csv(f"{output_folder}\\[{dubposBchans}] Positive Cells\\Total Intensity [{dubposBchans}].csv", sep=',')
    
        #Bar plot of double positive cell number per channel per dataset
    
        plt.figure()
        sns.barplot(data = dfdubposBnumlong, x = "Channel", y = "Number of Cells", hue = "Dataset")
        plt.title(f"Total Number of [{dubposBchans}] Positive Cells by Target Channel")
        plt.xticks(plt.xticks()[0])
        plt.legend()
        plt.savefig(f"{output_folder}\\[{dubposBchans}] Positive Cells\\Positive Cell Number [{dubposBchans}]")
        ...
        plt.figure()
        sns.barplot(data = dfdubposBnum_tot, x = "Dataset", y = "Value")
        plt.ylabel("Number of Cells")
        plt.title(f"Total Number of [{dubposBchans}] Positive Cells (All Channels)")
        plt.savefig(f"{output_folder}\\[{dubposBchans}] Positive Cells\\Total Positive Cells [{dubposBchans}]") 
